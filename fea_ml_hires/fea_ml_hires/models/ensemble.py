...
def train_ensemble_member(
    ...
    **kwargs,
    ...
    Features:
      ...
      - Gradient accumulation (via ``grad_accum_steps`` kwarg, default 1)
      - bf16 AMP (via ``use_amp`` kwarg, default True on CUDA)
    """
    import random
    ...
    # Gradient accumulation (for high-res grids where batch_size=1)
    grad_accum_steps = kwargs.get("grad_accum_steps", 1)
    use_amp = kwargs.get("use_amp", device.type == "cuda")

"""
Voxel-based dataset for HIGH-RESOLUTION FEA surrogate training (512^3 / 1024^3).

Key adaptations over the 128^3 version:
  - Memory-mapped NPZ loading: grids are loaded as np.memmap and only
    the needed bytes are read into memory on access
  - On-demand casting: occ/part stay as uint8 until __getitem__ builds
    the channel tensor, avoiding 7x memory overhead from one-hot encoding
    the entire grid up front
  - Chunked normalization stats: computes stats in mini-batches to avoid
    loading all targets into memory at once
  - Optional float16 voxel output (bf16 on GPU) to halve memory
  - Lazy SDF loading: SDF grids are only loaded if use_sdf=True and the
    file exists
...
class VoxelFEADataset(Dataset):
    """
    Dataset for high-resolution voxel-based FEA surrogate training.

    Designed for 512^3 / 1024^3 grids.  Each sample directory contains:
    ...
    Memory-efficiency:
      - Grids are loaded lazily per-sample (no full dataset in RAM)
      - One-hot encoding is done per-sample on the fly
      - Optional half-precision output via ``output_dtype``
    """
    ...
    def __init__(
        ...
        if len(self.valid_runs) < len(self.run_dirs):
            print(f"Warning: {len(self.run_dirs) - len(self.valid_runs)} "
                  f"invalid run directories skipped")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # --- Load grids (lazy, uint8 stays compact) ---
        occ = np.load(run_dir / "occ.npz")["data"]         # (D,H,W) uint8
        part = np.load(run_dir / "part.npz")["data"]        # (D,H,W) uint8
        ...
        return {
            "voxel": torch.from_numpy(voxel_input).to(self.output_dtype),
    ...
    def _build_voxel_input(
        ...
        """Build multi-channel voxel input: [occ, part_0..part_5, (sdf)]."""
        channels = [occ.astype(np.float32)[None, ...]]  # (1, D, H, W)
    ...
    def _augment(self, voxel: np.ndarray) -> np.ndarray:
        """Random augmentations that preserve structural meaning.

        For high-res grids, augmentation is kept lightweight to avoid
        copying ~7 GB arrays unnecessarily.
        """
        # Random 90-degree rotations around Z axis (axes 2,3 = H,W)
        k = np.random.randint(4)
        if k > 0:
            voxel = np.rot90(voxel, k=k, axes=(2, 3)).copy()
        ...
        # NOTE: Gaussian noise and channel dropout are skipped at high res
        # to avoid allocating GB-sized noise arrays.  The stochastic depth
        # and dropout inside the model provide sufficient regularisation.
...
def compute_voxel_normalization_stats(
    ...
    Only reads targets.json and meta.json (no voxel grids), so this is

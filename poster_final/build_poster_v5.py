#!/usr/bin/env python3
"""
ISEF Poster PPTX v5 — Final production build.
Uses all 20 generated figures from poster_final/.
Matches Rishab Kumar Jain reference poster style.

Usage:
    cd poster_final && python3 build_poster_v5.py

Output:  SASTO_ISEF_Poster_v5.pptx (48×36 in)
"""
...
# ═══════════════════════════════════════════════════════════════
# COLOR PALETTE (matches poster_final figures)
# ═══════════════════════════════════════════════════════════════
...
FIG_DIR = BASE  # poster_final/ — all generated figures here
OUTPUT = os.path.join(BASE, "SASTO_ISEF_Poster_v5.pptx")


def fig(name):
    """Find a generated figure in poster_final/."""
    p = os.path.join(FIG_DIR, name + ".png")
    return p if os.path.isfile(p) else None


def extracted_img(name):
    """Find extracted image from poster_images_extracted/."""
    for ext in (".jpg", ".png"):
...
def img_safe(sl, f, x, y, w, h):
    """Insert image respecting aspect ratio."""
    ...
    try:
        ...
        # Fallback: insert at exact dimensions
...
def tbl(sl, x, y, w, rows, col_pcts):
    """Table helper. Returns (table_shape, y_bottom)."""
...
def build():
    ...
    if va_fig:
        img_safe(sl, va_fig, LX + CP, y + CP, LW - 2*CP, 2.70)
    cap(sl, LX + CP, y + 2.80, LW - 2*CP,
        "Fig. 1. SASTO pipeline: wireframe -> parts -> FEA -> surrogate -> optimize -> STL")
    tb(sl, LX + CP, y + 3.02, LW - 2*CP, 0.85,
       "The visual abstract shows the end-to-end SASTO workflow. A 3DWire building wireframe "
       "is extruded into four structural parts, meshed and simulated with FEA to train a 5-member "
       "deep ensemble. At optimization time, the surrogate replaces FEA, enabling 23-92x faster "
       "topology optimization with zero constraint violations.",
    ...
    if f02:
        img_safe(sl, f02, ix, y + CP, iw, 4.80)
    cap(sl, ix, y + CP + 4.90, iw, "Uniform vs. SASTO optimized*")
    ...
    # Use generated table figure
    f18 = fig("fig18_design_criteria_table")
    if f18:
        img_safe(sl, f18, LX + CP, y + CP, LW - 2*CP, 1.60)
    ...
    tb(sl, LX + CP, ty, LW - 2*CP, 0.60,
       "Material: isotropic linear elastic concrete. E=25 GPa, v=0.20, rho=2,400 kg/m3, "
       "f'c=30 MPa. Loading: ASCE 7-22 ASD - dead (self-weight), live (1.92 kPa), "
       "lateral wind (0.96 kPa). Fixed-base BC at min-x face.",
    ...
    if f14:
        img_safe(sl, f14, LX + CP, py, LW - 2*CP, pf_h - (py - y) - CP - 0.30)
    ...
    # Use extracted renders if available
    ri_img = extracted_img("Image 11")
    if ri_img:
        img_safe(sl, ri_img, cx_arr[0], ry + 0.24, cw3, 2.80)
    badge(sl, cx_arr[0] + cw3/2 - 0.70, ry + 2.20, 1.40, 0.22, "-45.0% material", RED)
    cap(sl, cx_arr[0], ry + 3.08, cw3, "Before/after optimization renders")

    # Reference table
    f19 = fig("fig19_reference_table")
    if f19:
        img_safe(sl, f19, cx_arr[0], ry + 3.32, cw3, 2.40)
    ...
    # Use generated banner figure if available, else manual

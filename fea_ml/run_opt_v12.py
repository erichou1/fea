"""
V12: Sensitivity-Guided Topology Optimization (fixed)
...
Improvements over V10 (34.3% volume reduction):
  ...
  2. UNIFORM MIN_THICK=2: All parts (including interior walls) use
     MIN_THICK=2. V11's MIN_THICK=1 for interior walls was too
     aggressive (9.6% kept, holes in attic floor, shards).
...
print("[0] V12 script start", flush=True)
...
MIN_THICK_DEFAULT  = 2          # all parts uniform
MIN_THICK_INTERIOR = 2          # ← V12: back to 2 (V11's 1 was too aggressive)
BATCH_SIZE         = 200
...
def is_simple_point(vol, z, y, x):
    """26,6 digital topology check — True if removal preserves topology."""
    if vol[z, y, x] == 0:
        return False
    ...
    fg_labels, fg_count = label(nb, structure=_STRUCT26)
    if fg_count != 1:
        return False

    bg = (nb == 0).astype(np.uint8)
    bg_labels, _ = label(bg, structure=_STRUCT6)
    adj_labels = set()
...
def save_checkpoint(current_occ, result, batch_num, total_removed, history,
                    phase="erosion"):
    np.savez_compressed(OPT_OUT / "optimized_occ_v12.npz", data=current_occ)
    ...
    with open(OPT_OUT / "checkpoint_v12.json", "w") as f:
...
# ── remove spike voxels ──────────────────────────────────────────
print(f"\n[10] Removing shards (≤5 neighbors)...", flush=True)
kernel = _STRUCT26.astype(np.float32)
kernel[1, 1, 1] = 0
SHARD_THRESHOLD = 6   # remove voxels with < 6 neighbors (i.e., ≤5)
total_spikes = 0
for i in range(15):
...
print(f"\n{'='*60}", flush=True)
print("V12 OPTIMIZATION COMPLETE", flush=True)
print(f"{'='*60}", flush=True)
...
summary = {
    "success": True,
    "method": "v12_sensitivity_guided_fixed",
    "improvements": [
...
with open(OPT_OUT / "optimization_summary_v12.json", "w") as f:
...
try:
    export_mesh(current_occ, OPT_OUT / "optimized_v12_sharp.stl",
                "V12 sharp")
    export_mesh(current_occ, OPT_OUT / "optimized_v12_medium.stl",
                "V12 medium", blur_sigma=0.3, smooth_iter=5)
